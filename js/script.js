...
// === Drag and Swipe for Character Cards ===
const characterTrack = document.querySelector('.character-track');
let isDragging = false;
let startX = 0;
let scrollLeft = 0;

characterTrack.addEventListener('mousedown', (e) => {
  isDragging = true;
  startX = e.pageX - characterTrack.offsetLeft;
  scrollLeft = characterTrack.scrollLeft;
  characterTrack.style.cursor = 'grabbing';
});

characterTrack.addEventListener('mouseleave', () => {
  isDragging = false;
  characterTrack.style.cursor = 'grab';
});

characterTrack.addEventListener('mouseup', () => {
  isDragging = false;
  characterTrack.style.cursor = 'grab';
});

characterTrack.addEventListener('mousemove', (e) => {
  if (!isDragging) return;
  e.preventDefault();
  const x = e.pageX - characterTrack.offsetLeft;
  const walk = (x - startX) * 2; // Adjust scroll speed
  characterTrack.scrollLeft = scrollLeft - walk;
});

// Add touch support for mobile devices
characterTrack.addEventListener('touchstart', (e) => {
  isDragging = true;
  startX = e.touches[0].pageX - characterTrack.offsetLeft;
  scrollLeft = characterTrack.scrollLeft;
});

characterTrack.addEventListener('touchend', () => {
  isDragging = false;
});

characterTrack.addEventListener('touchmove', (e) => {
  if (!isDragging) return;
  const x = e.touches[0].pageX - characterTrack.offsetLeft;
  const walk = (x - startX) * 2; // Adjust scroll speed
  characterTrack.scrollLeft = scrollLeft - walk;
});